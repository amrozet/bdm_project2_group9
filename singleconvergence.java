...
public class singleconvergence {
	...
	public static class KCenterReducer extends Reducer<Text, Text, Text, Text> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			Float old_x=Float.parseFloat(list[0]);
			Float old_y=Float.parseFloat(list[1]);
			centerx = centerx / num;
			centery = centery / num;
			C_new_x.add(centerx);// maybe cannot get new
			// points
			C_new_y.add(centery);
			C_old_x.add(old_x);// maybe cannot get old
			// points
			C_old_y.add(old_y);
	...
	public static class DriverKMeans extends Configured implements Tool {
		...
		for (int i = 0; i < 6; i++) {
			...
			job.setJarByClass(singleconvergence.class);
