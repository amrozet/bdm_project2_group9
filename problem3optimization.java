...
public class problem3optimization {
		...
		public void map(LongWritable key, Text value, Context context)
			...
			for (i = 0; i < Cx.size(); i++) {
				...
				if (distance < mindistance) {
					...
					//System.err.println("youyou");
				}
			}
			
			//System.err.println(winnercenter);
	...
	public static class kmeancombiner 
	 extends Reducer<Text, Text,Text,Text> {
		  ...
		  public void reduce(Text key, Iterable<Text> values , 
			  ...
			  //System.err.println(Float.toString(centerx)+","+Float.toString(centery)+","+num);
	...
	public static class KCenterReducer extends Reducer<Text, Text, Text, Text> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			context.write(newCenter, new Text("")); // key: old center; value:
		}
	}
	...
	public static class DriverKMeans extends Configured implements Tool {
		...
		for (int i = 0; i < 6; i++) {
			...
			job.setJarByClass(problem3optimization.class);
			...
			if (temp_sum < 100) {
				System.err.println("conver");
				/*FileSystem hdfs1 = FileSystem.get(new Configuration());
				Path file1 = new Path(output + OUTPUT_FILE_NAME); // second output
				BufferedWriter brwrite = new BufferedWriter(new OutputStreamWriter(
						hdfs1.append(file1)));
				brwrite.write("converage");
				*/
				FileWriter fw = new FileWriter(output + OUTPUT_FILE_NAME, true);
				System.err.println(output + OUTPUT_FILE_NAME);
			    BufferedWriter bw = new BufferedWriter(fw);
			    PrintWriter out = new PrintWriter(bw);
			    out.println("KMeans clustering is convergent.");
			    out.close();
				break;
			}
			if(i==5){
				System.err.println("NOT conver");
				//FileSystem hdfs1 = FileSystem.get(new Configuration());
				//FileSystem hdfs1 = FileSystem.get(new Configuration());
				//Path file1 = new Path(output + OUTPUT_FILE_NAME); // second output
				//BufferedWriter brwrite = new BufferedWriter(file1);
				//brwrite.write("not converage");
				FileWriter fw = new FileWriter(output + OUTPUT_FILE_NAME, true);
			    BufferedWriter bw = new BufferedWriter(fw);
			    PrintWriter out = new PrintWriter(bw);
			    out.println("KMeans clustering is not convergent");
			    out.close();
